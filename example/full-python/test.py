...
def test_create():
    ...
    for options in (tuple(itertools.chain(*v)) for v in itertools.product(
        ...
        with tempfile.NamedTemporaryFile() as f:
            ...
            #subprocess.run(cmd, cwd=this_dir, check=True)
            lines = subprocess.run(cmd, stdout=subprocess.PIPE, cwd=this_dir, check=True).stdout.decode("utf8").splitlines()
...
def assert_dir(d, file_name_args, file_names):
    ...
    for arg, name in zip(file_name_args, file_names):
        ...
        assert read_file(os.path.join(this_dir, source_path)) == read_file(os.path.join(d, name)), name
